...
func DialKCP(ctx context.Context, dest net.Destination, streamSettings *internet.MemoryStreamConfig) (stat.Connection, error) {
	...
	if streamSettings.UdpmaskManager != nil {
		...
		case *internet.PacketConnWrapper:
			pktConn, err := streamSettings.UdpmaskManager.WrapPacketConnClient(c.PacketConn, streamSettings.SocketSettings)
			if err != nil {
				conn.Close()
		...
		case *net.UDPConn:
			pktConn, err := streamSettings.UdpmaskManager.WrapPacketConnClient(c, streamSettings.SocketSettings)
			if err != nil {
				conn.Close()
				return nil, errors.New("mask err").Base(err)
			}
		...
		case *cnc.Connection:
			...
			pktConn, err := streamSettings.UdpmaskManager.WrapPacketConnClient(fakeConn, streamSettings.SocketSettings)
			if err != nil {
				conn.Close()
				return nil, errors.New("mask err").Base(err)
			}
